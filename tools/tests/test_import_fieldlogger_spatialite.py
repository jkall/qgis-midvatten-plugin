...
class TestFieldLoggerImporterDb(utils_for_tests.MidvattenTestSpatialiteDbSv):
    ...
    def test_full_integration_test_to_db_change_datetimefilter_after_update_button(self):
        ...
        with common_utils.tempinput(''.join(f)) as filename:
            ...
            t.update(dict([(k, db_utils.sql_load_fr_db('SELECT * FROM %s ORDER BY obsid, date_time, parameter' % k)) for k in ('w_qual_field',)]))
            test_string = create_test_string(t)
            reference_string = '{comments: (True, [(Rb1202, 2016-03-30 15:31:30, hej2, teststaff, None)]), w_flow: (True, [(Rb1615, testid, Accvol, 2016-03-30 15:30:09, 357.0, m3, gick bra)]), w_levels: (True, []), w_qual_field: (True, [(Rb1505, teststaff, 2016-03-30 15:29:26, testid, konduktivitet, 863.0, 863, µS/cm, None, hej), (Rb1512, teststaff, 2016-03-30 15:30:39, testid, syre, 67.0, 67, mg/l, None, test), (Rb1512, teststaff, 2016-03-30 15:30:39, testid, temperatur, 8.0, 8, grC, None, test), (Rb1512, teststaff, 2016-03-30 15:30:40, testid, syre, 58.0, 58, %, None, None), (Rb1512, teststaff, 2016-03-30 15:31:30, testid, turbiditet, 899.0, 899, FNU, None, None)])}'
            print(test_string)
            print("REF")
            print(reference_string)
            assert test_string == reference_string
    ...
    def test_full_into_zz_flowtype(self):
        ...
        with common_utils.tempinput(''.join(f)) as filename:
            ...
            test_string = create_test_string(dict([(k, db_utils.sql_load_fr_db('SELECT * FROM %s' % k)) for k in ('w_flow', 'comments', 'zz_flowtype')]))
            reference_string = '{comments: (True, [(2, 2016-12-12 10:03:15, onlycomment, teststaff, None)]), w_flow: (True, [(2, testid, Momflow2, 2016-12-12 10:03:07, 123.0, aunit, None)]), zz_flowtype: (True, [(Accvol, Accumulated volume), (Momflow, Momentary flow rate), (Aveflow, Average flow since last reading), (Momflow2, None)])}'
            assert test_string == reference_string
    ...
    def test_full_integration_test_to_db_w_levels_value_calculate_level_masl(self):
        ...
        with common_utils.tempinput(''.join(f)) as filename:
            ...
            test_string = create_test_string(dict([(k, db_utils.sql_load_fr_db('select * from %s'%k)) for k in ('w_levels', 'w_qual_field', 'w_flow', 'comments')]))
            print(test_string)
            reference_string = '{comments: (True, [(Rb1608, 2016-03-30 15:34:40, testc, teststaff, None), (Rb1615, 2016-03-29 15:34:13, ergv2, teststaff, None)]), w_flow: (True, []), w_levels: (True, [(Rb1608, 2016-03-30 15:34:13, 555.0, 0.0, -555.0, ergv), (Rb1615, 2016-03-31 15:34:13, 111.0, None, None, ergv1)]), w_qual_field: (True, [])}'
            assert test_string == reference_string
