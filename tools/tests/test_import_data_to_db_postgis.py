...
class TestGeneralImport(utils_for_tests.MidvattenTestPostgisDbSvImportInstance):
    ...
    def test_general_import_wlvllogg_missing_not_null_column(self, mock_iface,
        ...
        nose.tools.assert_raises(MidvDataImporterError,
...
class TestImportObsPointsObsLines(
    ...
    def test_import_obsids_directly(self):
        ...
        result = db_utils.sql_load_fr_db('select obsid, name, place, type, length, drillstop, diam, material, screen, capacity, drilldate, wmeas_yn, wlogg_yn, east, north, ne_accur, ne_source, h_toc, h_tocags, h_gs, h_accur, h_syst, h_source, source, com_onerow, com_html, geometry FROM obs_points')
        assert result == (True, [('obsid1', None, None, None, None, None, None, None,
                                  None, None, None, None, None, None, None, None, None,
                                  None, None, None, None, None, None, None, None, None,
                                  None), (
                                 'obsid2', None, None, None, None, None, None, None,
                                 None, None, None, None, None, None, None, None, None,
                                 None, None, None, None, None, None, None, None, None,
                                 None)])
    ...
    def test_import_obs_points_already_exist(self, mock_iface):
        ...
        self.importinstance.general_import(file_data=f, dest_table='obs_points')
        assert call.messageBar().createMessage(
            '0 rows imported and 1 excluded for table obs_points. See log message panel for details') in mock_iface.mock_calls
    ...
    def test_import_obs_lines_geometry_as_wkt(self):
        ...
        test_string = utils_for_tests.create_test_string(db_utils.sql_load_fr_db(
            '''select obsid, ST_AsText(geometry) from obs_lines'''))
...
class TestWquallabImport(utils_for_tests.MidvattenTestPostgisDbSvImportInstance):
    ...
    def test_wquallab_import_from_csvlayer(self):
        db_utils.sql_alter_db("INSERT INTO obs_points (obsid) VALUES ('obsid1')")
        f = [['obsid', 'depth', 'report', 'project', 'staff', 'date_time', 'anameth',
              'parameter', 'reading_num', 'reading_txt', 'unit', 'comment'],
             ['obsid1', '2', 'testreport', 'testproject', 'teststaff',
              '2011-10-19 12:30:00', 'testmethod', '1,2-Dikloretan', '1.5', '<1.5',
              'µg/l', 'testcomment']]

        self.importinstance.general_import(dest_table='w_qual_lab', file_data=f)

        test_string = utils_for_tests.create_test_string(
            db_utils.sql_load_fr_db('''select obsid, depth, report, project, staff, date_time, anameth, parameter, reading_num, reading_txt, unit, comment FROM  w_qual_lab'''))
    ...
    def test_wquallab_import_from_csvlayer_depth_empty_string(self):
        ...
        f = [['obsid', 'depth', 'report', 'project', 'staff', 'date_time', 'anameth',
              'parameter', 'reading_num', 'reading_txt', 'unit', 'comment'],
             ['obsid1', '', 'testreport', 'testproject', 'teststaff',
              '2011-10-19 12:30:00', 'testmethod', '1,2-Dikloretan', '1.5', '<1.5',
              'µg/l', 'testcomment']]
    ...
    def test_wquallab_import_from_csvlayer_no_staff(self):
        ...
        f = [
            ['obsid', 'depth', 'report', 'project', 'date_time', 'anameth', 'parameter',
             'reading_num', 'reading_txt', 'unit', 'comment'],
            ['obsid1', '2', 'testreport', 'testproject', '2011-10-19 12:30:00',
             'testmethod', '1,2-Dikloretan', '1.5', '<1.5', 'µg/l', 'testcomment']]
...
class TestWflowImport(utils_for_tests.MidvattenTestPostgisDbSvImportInstance):
    ...
    def test_wflow_import_from_csvlayer(self):
        db_utils.sql_alter_db("""INSERT INTO obs_points (obsid) VALUES ('obsid1')""")
        f = [['obsid', 'instrumentid', 'flowtype', 'date_time', 'reading', 'unit',
              'comment'],
             ['obsid1', 'testid', 'Momflow', '2011-10-19 12:30:00', '2', 'l/s',
              'testcomment']]

        self.importinstance.general_import(dest_table='w_flow', file_data=f)

        test_string = utils_for_tests.create_test_string(
            db_utils.sql_load_fr_db('''select obsid, instrumentid, flowtype, date_time, reading, unit, comment FROM w_flow'''))
    ...
    def test_wflow_import_from_csvlayer_type_missing(self):
        ...
        f = [['obsid', 'instrumentid', 'flowtype', 'date_time', 'reading', 'unit',
              'comment'],
             ['obsid1', 'testid', 'Testtype', '2011-10-19 12:30:00', '2', 'l/s',
              'testcomment']]
    ...
    def test_wflow_new_param_into_zz_flowtype(self):
        ...
        f = [['obsid', 'instrumentid', 'flowtype', 'date_time', 'reading', 'unit',
              'comment'],
             ['obsid1', 'testid', 'Momflow2', '2011-10-19 12:30:00', '2', 'l/s',
              'testcomment']]
        ...
        assert test_string == reference_string
        test_string = utils_for_tests.create_test_string(
            db_utils.sql_load_fr_db('''select type, explanation from zz_flowtype'''))
...
class TestWqualfieldImport(utils_for_tests.MidvattenTestPostgisDbSvImportInstance):
    ...
    def test_w_qual_field_import_from_csvlayer(self):
        db_utils.sql_alter_db("""INSERT INTO obs_points (obsid) VALUES ('obsid1')""")
        f = [['obsid', 'staff', 'date_time', 'instrument', 'parameter', 'reading_num',
              'reading_txt', 'unit', 'depth', 'comment'],
             ['obsid1', 'teststaff', '2011-10-19 12:30:00', 'testinstrument', 'DO',
              '12', '<12', '%', '22', 'testcomment']]

        self.importinstance.general_import(dest_table='w_qual_field', file_data=f)

        test_string = utils_for_tests.create_test_string(
            db_utils.sql_load_fr_db('''select obsid, staff, date_time, instrument, parameter, reading_num, reading_txt, unit, depth, comment FROM  w_qual_field'''))
    ...
    def test_w_qual_field_no_parameter(self, mock_messagebar):
        ...
        assert str(
            ex) == 'Required columns parameter are missing for table w_qual_field'
    ...
    def test_w_qual_field_parameter_empty_string(self, mock_messagebar):
        ...
        f = [['obsid', 'staff', 'date_time', 'instrument', 'parameter', 'reading_num',
             ...
             ['obsid1', 'teststaff', '2011-10-19 12:30:00', 'testinstrument', 'DO',
              '12', '<12', '%', '22', 'testcomment'],
             ['obsid2', 'teststaff', '2011-10-19 12:30:00', 'testinstrument', '', '12',
              '<12', '%', '22', 'testcomment']]
        ...
        test_calls_list = [
            call.info(
                bar_msg='1 rows imported and 1 excluded for table w_qual_field. See log message panel for details',
                log_msg='--------------------')]
    ...
    def test_w_qual_field_staff_null(self, mock_messagebar):
        ...
        f = [['obsid', 'staff', 'date_time', 'instrument', 'parameter', 'reading_num',
              'reading_txt', 'unit', 'depth', 'comment'],
             ['obsid1', '', '2011-10-19 12:30:00', 'testinstrument', 'DO', '12', '<12',
              '%', '22', 'testcomment'],
             ['obsid2', '', '2011-10-19 12:30:00', 'testinstrument', 'DO', '12', '<12',
              '%', '22', 'testcomment']]

        self.importinstance.general_import(dest_table='w_qual_field', file_data=f)

        test_calls_list = [call.info(
                               bar_msg='2 rows imported and 0 excluded for table w_qual_field. See log message panel for details',
                               log_msg='--------------------')]
        for test_call in test_calls_list:
            assert test_call in mock_messagebar.mock_calls
    ...
    def test_w_qual_field_units(self, mock_messagebar):
        ...
        f = [['obsid', 'staff', 'date_time', 'instrument', 'parameter', 'reading_num',
              'reading_txt', 'unit', 'depth', 'comment'],
             ['obsid1', 'teststaff', '2011-10-19 12:30:00', 'testinstrument', 'DO',
              '12', '<12', 'test unit', '22', 'testcomment'],
             ['obsid2', 'teststaff', '2011-10-19 12:31:00', 'testinstrument2', 'DO2',
              '13', '<13', 'test unit  ', '23', 'testcomment'],
             ['obsid2', 'teststaff', '2011-10-19 12:36:00', 'testinstrument2', 'DO2',
              '13', '<13', '  test  unit  ', '23', 'testcomment'],
             ['obsid3', 'teststaff', '2011-10-19 12:32:00', 'testinstrument3', 'DO3',
              '14', '<14', '  test unit  ', '24', 'testcomment'],
             ['obsid3', 'teststaff', '2011-10-19 12:33:00', 'testinstrument3', 'DO3',
              '14', '<14', None, '24', 'testcomment'],
             ['obsid3', 'teststaff', '2011-10-19 12:34:00', 'testinstrument3', 'DO3',
              '14', '<14', ' ', '24', 'testcomment'],
             ['obsid3', 'teststaff', '2011-10-19 12:35:00', 'testinstrument3', 'DO3',
              '14', '<14', '  ', '24', 'testcomment']]

        self.importinstance.general_import(dest_table='w_qual_field', file_data=f)

        print(str(db_utils.sql_load_fr_db('''SELECT obsid, staff, date_time, instrument, parameter, reading_num, reading_txt, unit, depth, comment FROM  w_qual_field''')))
        test_calls_list = [
            call.info(
                bar_msg='7 rows imported and 0 excluded for table w_qual_field. See log message panel for details',
                log_msg='--------------------')]
        print(str(mock_messagebar.mock_calls))
        for test_call in test_calls_list:
            assert test_call in mock_messagebar.mock_calls
...
class TestSeismicImport(utils_for_tests.MidvattenTestPostgisDbSvImportInstance):
    ...
    def test_seismic_import_from_csvlayer(self):
        ...
        test_string = utils_for_tests.create_test_string(
            db_utils.sql_load_fr_db('''select obsid, length, ground, bedrock, gw_table, comment FROM seismic_data'''))
...
class TestStratImport(utils_for_tests.MidvattenTestPostgisDbSvImportInstance):
    ...
    def test_strat_import_from_csvlayer(self):
        db_utils.sql_alter_db("""INSERT INTO obs_points (obsid) VALUES ('obsid1')""")
        f = [['obsid', 'stratid', 'depthtop', 'depthbot', 'geology', 'geoshort',
              'capacity', 'development', 'comment'],
             ['obsid1', '1', '0', '1', 'grusig sand', 'sand', '5', '(j)', 'acomment'],
             ['obsid1', '2', '1', '4', 'siltigt sandigt grus', 'grus', '4+', '(j)',
              'acomment2']]

        self.importinstance.general_import(dest_table='stratigraphy',
                                           file_data=f)  # dest_table='stratigraphy')

        test_string = utils_for_tests.create_test_string(
            db_utils.sql_load_fr_db('''select obsid, stratid, depthtop, depthbot, geology, geoshort, capacity, development, comment FROM stratigraphy'''))
    ...
    def test_strat_import_one_obs_fail_stratid_gaps(self):
        ...
        f = [['obsid', 'stratid', 'depthtop', 'depthbot', 'geology', 'geoshort',
             ...
             ['obsid2', '3', '1', '4', 'siltigt sandigt grus', 'grus', '4+', '(j)',
              'acomment2']]
    ...
    def test_strat_import_one_obs_fail_depthbot_gaps(self):
        ...
        f = [['obsid', 'stratid', 'depthtop', 'depthbot', 'geology', 'geoshort',
             ...
             ['obsid1', '2', '1', '4', 'siltigt sandigt grus', 'grus', '4+', '(j)',
              'acomment2'],
             ['obsid2', '1', '0', '1', 'grusig sand', 'sand', '5', '(j)', 'acomment'],
             ['obsid2', '2', '3', '4', 'siltigt sandigt grus', 'grus', '4+', '(j)',
              'acomment2']]
...
class TestMeteoImport(utils_for_tests.MidvattenTestPostgisDbSvImportInstance):
    ...
    def test_meteo_import_from_csvlayer(self):
        db_utils.sql_alter_db("""INSERT INTO obs_points (obsid) VALUES ('obsid1')""")
        f = [['obsid', 'instrumentid', 'parameter', 'date_time', 'reading_num',
              'reading_txt', 'unit', 'comment'],
             ['obsid1', 'ints1', 'pressure', '2016-01-01 00:00:00', '1100', '1100',
              'aunit', 'acomment']]
...
class TestObsLinesImport(utils_for_tests.MidvattenTestPostgisDbSvImportInstance):
    ...
    def test_obs_lines_import_from_csvlayer(self, mock_messagebar):
        ...
        test_string = utils_for_tests.create_test_string(
            db_utils.sql_load_fr_db('''select obsid, name, place, type, source, geometry FROM obs_lines'''))
...
class TestDeleteExistingDateTimesFromTemptable(
    ...
    def test_delete_existing_date_times_from_temptable_minute_already_exists(self):
        ...
        db_utils.sql_alter_db("""INSERT INTO obs_points (obsid) VALUES ('obsid1')""")
        db_utils.sql_alter_db(
            """INSERT INTO w_levels (obsid, date_time, level_masl) VALUES ('obsid1', '2016-01-01 00:00', '123.0')""")
