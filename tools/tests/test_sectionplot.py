...
class TestSectionPlot(object):
    ...
    def setUp(self, mock_savefilename, mock_crsquestion, mock_qgsproject_instance, mock_locale):
        ...
        #Obs_lines have to be initiated here.
        utils.sql_alter_db(u'''insert into obs_lines (obsid, geometry) values ("L1", GeomFromText('LINESTRING(0 0, 1 0, 10 0)', 3006))''')
        utils.sql_alter_db(u'''insert into obs_lines (obsid, geometry) values ("L2", GeomFromText('LINESTRING(2 0, 10 10)', 3006))''')
        self.uri = TestSectionPlot.uri
        self.uri.setDatabase(TEMP_DB_PATH)
        self.uri.setDataSource('', 'obs_lines', 'geometry', '', 'obsid')
    ...
    @mock.patch('midvatten_utils.QgsProject.instance')
    def _test_plot_section_selected(self, mock_qgsproject_instance):
        mock_qgsproject_instance.return_value.readEntry = MIDV_DICT
    ...
    def test_plot_section_length_along(self, mock_qgsproject_instance):
        ...
        vlayer = QgsVectorLayer(self.uri.uri(), 'TestLayer', 'spatialite')
        features = vlayer.getFeatures()

        for feature in features:
            featureid = feature.id()
            attrs = [x.toString() for x in feature.attributes()]
            if attrs[0] == u'L1':
                break
        ...
        @mock.patch('sectionplot.utils.pop_up_info')
        @mock.patch('midvatten_utils.getselectedobjectnames', autospec=True)
        @mock.patch('qgis.utils.iface', autospec=True)
        def _test(self, vlayer, mock_iface, mock_getselectedobjectnames, mock_skippopup):
    ...
    @mock.patch('midvatten_utils.QgsProject.instance')
    def _test_plot_section_length_along_slope(self, mock_qgsproject_instance):
        mock_qgsproject_instance.return_value.readEntry = MIDV_DICT
        ...
        print("TODO: In db before: " + str(utils.sql_load_fr_db(u'select obsid, AsText(geometry) from obs_lines')))
        ...
        test_string = utils_for_tests.create_test_string(myplot.LengthAlong)
        print("test_plot_section_length_along_slope" + test_string)
