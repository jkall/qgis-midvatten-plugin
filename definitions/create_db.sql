...
SPATIALITE CREATE VIEW obs_p_w_lvl_logger AS SELECT a.rowid, a.obsid, a.geometry FROM obs_points a WHERE EXISTS (SELECT obsid FROM w_levels_logger b WHERE b.obsid = a.obsid LIMIT 1);
...
SPATIALITE INSERT INTO views_geometry_columns (view_name, view_geometry, view_rowid, f_table_name, f_geometry_column, read_only) VALUES ('obs_p_w_lvl_logger', 'geometry', 'rowid', 'obs_points', 'geometry',1);
...
POSTGIS CREATE VIEW obs_p_w_lvl_logger AS SELECT a.obsid, a.geometry FROM obs_points a WHERE EXISTS (SELECT obsid FROM w_levels_logger b WHERE b.obsid = a.obsid LIMIT 1);
