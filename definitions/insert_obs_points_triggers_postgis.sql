...
CREATE FUNCTION after_insert_obs_points_coords_fr_geom()
  RETURNS trigger AS
$BODY$
BEGIN
  UPDATE obs_points SET  east = X(geometry), north = Y(geometry)
  WHERE (NEW.east is null) AND (NEW.north is null) AND (NEW.geometry is not NULL) AND (NEW.obsid = obsid);
  RETURN NEW;
 END;
$BODY$
LANGUAGE plpgsql VOLATILE
...
CREATE TRIGGER trigger_after_insert_obs_points_coords_fr_geom
  AFTER INSERT ON obs_points
  FOR EACH ROW
  WHEN (0 < (select count() from obs_points where ((NEW.east is null) AND (NEW.north is null) AND (NEW.geometry is not NULL))))
  EXECUTE PROCEDURE after_insert_obs_points_coords_fr_geom();
#----------------------------------------------------------------------------------
...
CREATE FUNCTION after_update_obs_points_geom_fr_coords()
  RETURNS trigger AS
$BODY$
BEGIN
  UPDATE obs_points SET geometry = MakePoint(NEW.east, NEW.north, (select srid from geometry_columns where f_table_name = 'obs_points'))
  WHERE ((NEW.east != OLD.east) OR (NEW.north != OLD.north) OR (NEW.north IS NOT NULL AND OLD.north IS NULL) OR (NEW.east IS NOT NULL AND OLD.east IS NULL)) AND (NEW.obsid = obsid) AND (NEW.east is not null) AND (NEW.north is not null);
  RETURN NEW;
 END;
$BODY$
LANGUAGE plpgsql VOLATILE
...
CREATE TRIGGER trigger_after_update_obs_points_geom_fr_coords
  AFTER UPDATE ON obs_points
  FOR EACH ROW
  WHEN (0 < (select count() from obs_points where ((NEW.east != OLD.east) OR (NEW.north != OLD.north)) OR (NEW.north IS NOT NULL AND OLD.north IS NULL) OR (NEW.east IS NOT NULL AND OLD.east IS NULL)) )
  EXECUTE PROCEDURE after_update_obs_points_geom_fr_coords();
...
CREATE FUNCTION after_update_obs_points_coords_fr_geom()
  RETURNS trigger AS
$BODY$
BEGIN
  UPDATE obs_points SET east = X(geometry), north = Y(geometry)
  WHERE (NEW.geometry != OLD.geometry) AND (NEW.obsid = obsid);
  RETURN NEW;
 END;
$BODY$
LANGUAGE plpgsql VOLATILE
...
CREATE TRIGGER trigger_after_update_obs_points_coords_fr_geom
  AFTER UPDATE ON obs_points
  FOR EACH ROW
